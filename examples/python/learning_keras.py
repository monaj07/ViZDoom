...
def create_network(available_actions_count):
    input_state = Input(shape=(1, resolution[0], resolution[1]))
    conv1 = Convolution2D(nb_filter=8, nb_row=6, nb_col=6, subsample=(3, 3), init='glorot_normal', border_mode='same')(input_state)
    conv1 = Activation('relu')(conv1)
    conv2 = Convolution2D(nb_filter=8, nb_row=3, nb_col=3, subsample=(2, 2), init='glorot_normal', border_mode='same')(conv1)
    conv2 = Activation('relu')(conv2)
    conv2_flattened = Flatten()(conv2)
    dense = Dense(output_dim=128, init='glorot_normal')(conv2_flattened)
...
def perform_learning_step(epoch, loss):
    ...
    s1_4d = np.zeros(shape=(1, 1, resolution[0], resolution[1]))
    s1_4d[0, 0, :, :] = s1
...
for epoch in range(epochs):
    ...
    for test_episode in trange(test_episodes_per_epoch):
        ...
        while not game.is_episode_finished():
            ...
            state_4d = np.zeros(shape=(1, 1, resolution[0], resolution[1]))
            state_4d[0, 0, :, :] = state
            best_action_index = np.argmax(model.predict(state_4d))
